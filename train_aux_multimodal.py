...
# Local dependencies
from drim.trainers import AuxSurvivalTrainer
...
def main(cfg: DictConfig) -> None:
    ...
    if "wandb" in cfg:
        ...
        wandb_logging = True
        wandb.init(
            name=f"aux_{cfg.aux_loss.name}_{cfg.fusion.name}_"
            + "_".join(cfg.general.modalities),
            config={
    ...
    for fold in range(cfg.general.n_folds):
        ...
        cfg.general.save_path = (
            f"./models/aux_{cfg.aux_loss.name}_{cfg.fusion.name}_split_{int(fold)}.pth"
        )
        ...
        if cfg.aux_loss.name == "contrastive":
            from drim.losses import ContrastiveLoss

            aux_loss = ContrastiveLoss()
        elif cfg.aux_loss.name == "mmo":
            from drim.losses import MMOLoss

            aux_loss = MMOLoss()

        trainer = AuxSurvivalTrainer(
            model=model,
            optimizer=optimizer,
            scheduler=scheduler,
